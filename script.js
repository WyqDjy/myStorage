...
class Firework {
    ...
    reset() {
        ...
        this.trail = []; // 轨迹点数组
        this.particleCount = isMobile ? 30 : 80; // 进一步减少移动端粒子数量
    }
    ...
    update() {
        if (!this.exploded) {
            ...
            if (this.trail.length > 10) {
                this.trail.shift(); // 保持轨迹点数量不超过10个
            }
    ...
    explode() {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            const particle = {
                ...
                alpha: 1, // 初始透明度
                size: isMobile ? 1.5 + Math.random() * 1.5 : 3 + Math.random() * 3, // 减少移动端粒子大小
    ...
    draw() {
        if (!this.exploded) {
            ...
            for (const particle of this.particles) {
                ...
                // 绘制粒子尾迹（移动端简化）
                if (!isMobile && particle.trail.length > 1) {
                    this.ctx.beginPath();
...
function init() {
    ...
    // 创建星空
    const stars = [];
    const starCount = isMobile ? 80 : 200; // 进一步减少移动端星星数量
    ...
    // 创建烟花
    const fireworks = [];
    const fireworkCount = isMobile ? 3 : 10; // 进一步减少移动端烟花数量
    ...
    welcomeScreen.addEventListener('click', (e) => {
        e.preventDefault();
        if (e.target === welcomeScreen || e.target === startButton) {
            welcomeScreen.classList.add('hidden');
            // 开始播放动画
            animate();
        }
    ...
    let musicLoopTimer = null; // 用于存储音乐循环定时器的ID

    // 自动播放音乐函数
    function autoPlayMusic() {
        if (!isMusicPlaying && welcomeScreen.classList.contains('hidden')) {
            // 播放音乐
            ...
            const playBirthdayMelody = () => {
                ...
                // 循环播放 - 保存定时器ID
                musicLoopTimer = setTimeout(playBirthdayMelody, (currentTime - audioContext.currentTime) * 1000);
            };
